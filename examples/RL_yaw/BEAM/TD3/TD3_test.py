...
def test_td3(apply_disturbance = True):
    # Load environment with rendering enabled
    env = BallAndBeamPIDEnv(render_mode="human")
    
    # Load the trained model
    model = TD3.load("examples/RL_yaw/BEAM/TD3/td3_ball_beam_model")
    ...
    while step < 1500:
        ...
        if terminated:
            ...
            continue
            # Removed step = 0 to avoid potential infinite loop
        step += 1
    env.close()
...
if __name__ == "__main__":
    ...
    # train_sac()
    test_td3(apply_disturbance = True)
...
use crate::protocol::pair::{
    PairACK, PairCommand, PairRTCMSetOutputAntPnt, PairRTCMSetOutputEphemeris,
    PairRTCMSetOutputMode, PairRequestAiding, PairResponse,
};
...
mod tests {
    ...
    fn parses_full_pair010_aiding_request() {
        let resp = PairResponse::from_sentence("$PAIR010,0,0,2044,369413*33").unwrap();
        let PairResponse::RequestAiding(aiding) = resp else {
            panic!("expected PAIR010 request aiding response");
        };

        assert!(matches!(aiding.aiding_type, AidingType::EpoData));
        assert!(matches!(aiding.gnss_system, GnssSystem::Gps));
        assert_eq!(aiding.week_number, 2044);
        assert_eq!(aiding.time_of_week, 369413);
    }

    #[test]
    fn parses_short_pair010_time_and_location_requests() {
        for sentence in ["$PAIR010,1,-1*16", "$PAIR010,2,-1*15"] {
            let resp = PairResponse::from_sentence(sentence).unwrap();
            let PairResponse::RequestAiding(aiding) = resp else {
                panic!("expected PAIR010 request aiding response");
            };

            assert!(matches!(aiding.gnss_system, GnssSystem::NotApplicable));
            assert_eq!(aiding.week_number, 0);
            assert_eq!(aiding.time_of_week, 0);
        }
    }
}

...
pub struct Display {
    ...
    last_ml_prediction: Option<(String, String)>, // (Terrain, Confidence/Agreement)
}
...
impl Display {
    ...
    pub fn update_ml<W: Write>(
        ...
        self.last_ml_prediction = Some((terrain, confidence));
        self.render(stdout)
    }
    ...
    fn render<W: Write>(&mut self, stdout: &mut W) -> std::io::Result<()> {
        ...
        if let Some((terrain, confidence)) = &self.last_ml_prediction {
            items.push(DisplayItem::Header(

...
impl Display {
    ...
    fn render<W: Write>(&mut self, stdout: &mut W) -> std::io::Result<()> {
        ...
        if let Some(arduino_data) = &self.last_arduino {
            ...
            items.push(DisplayItem::Data(
                "Gyro Acc Lin X".to_string(),
                fmt_f32(arduino_data.gyro_x),
                Some("°/s".to_string()),
            ));
            items.push(DisplayItem::Data(
                "Gyro Acc Lin Y".to_string(),
                fmt_f32(arduino_data.gyro_y),
                Some("°/s".to_string()),
            ));
            items.push(DisplayItem::Data(
                "Gyro Acc Lin Z".to_string(),
                fmt_f32(arduino_data.gyro_z),
                Some("°/s".to_string()),
            ));
